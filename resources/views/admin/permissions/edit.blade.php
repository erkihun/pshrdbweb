...
@section('content')
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-brand-ink">Edit permission</h1>
            <p class="text-sm text-brand-muted">Rename {{ $permission->name }}.</p>
        </div>
        <a href="{{ route('admin.permissions.index') }}" class="text-sm font-semibold text-brand-blue">Back to permissions</a>
    </div>

    <form method="POST" action="{{ route('admin.permissions.update', $permission) }}" class="mt-6 space-y-6 rounded-2xl border border-brand-border bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
