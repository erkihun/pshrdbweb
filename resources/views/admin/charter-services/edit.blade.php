...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-slate-900">{{ __('common.citizen_charter.admin.actions.edit_title', ['name' => $service->display_name]) }}</h1>
                <p class="text-sm text-slate-500">{{ __('common.citizen_charter.admin.actions.edit_description') }}</p>
            </div>
            <a
        ...
        <form method="POST" action="{{ route('admin.charter-services.update', $service) }}">
            @csrf
            @method('PUT')
