...
@section('content')
    <div class="flex items-center justify-between">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-brand-ink">Edit role</h1>
            <p class="text-sm text-brand-muted">Adjust permissions for {{ $role->name }}.</p>
        </div>
        <a href="{{ route('admin.roles.index') }}" class="text-sm font-semibold text-brand-blue">Back to roles</a>
    </div>

    <form method="POST" action="{{ route('admin.roles.update', $role) }}" class="mt-6 space-y-6 rounded-2xl border border-brand-border bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <p class="text-xs text-brand-muted">Grant or revoke permissions for this role.</p>
