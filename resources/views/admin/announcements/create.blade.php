...
@section('content')
    <div class="space-y-6">
        <div class="rounded-xl bg-white p-6 shadow-sm">
            <h1 class="text-2xl font-semibold text-slate-900">{{ __('vacancies.application.announcement_create_title') }}</h1>
            <p class="text-sm text-slate-500">{{ __('vacancies.application.announcement_create_description') }}</p>
        </div>

        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.announcements.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @include('admin.posts._form', ['fixedType' => 'announcement'])
