                ...
                <div x-show="tab === 'overview'">
                    ...
                    <div class="mt-6 grid gap-4 sm:grid-cols-3">
                        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                            ...
                            <p class="mt-1 text-sm text-slate-700">{{ $organization->code ?? 'N/A' }}</p>
                        </div>
                        <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <div x-show="tab === 'stats'">
                    <div class="grid gap-6 lg:grid-cols-[1.35fr,0.65fr]">
                        <div class="space-y-4">
                            <form method="GET" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                ...
                                <div class="mt-4 flex flex-wrap items-center gap-2">
                                    ...
                                    <a href="{{ route('admin.organizations.show', $organization) }}" class="text-xs font-semibold uppercase tracking-wide text-slate-500 hover:text-slate-700">{{ __('common.admin_organizations.filters.reset') }}</a>
                                </div>
                            </form>
                                                        ...
                                                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                            <div>
                                                                ...
                                                                <p class="text-xs text-slate-500">
                                                                    ...
                                                                    @endif
                                                                    @if($segment['month'])
                                                                        {{ __('common.admin_organizations.filters.month_label', ['month' => \Carbon\Carbon::create(null, $segment['month'])->format('F')]) }}
                                                                    @endif
                                                                </p>
                                                            </div>
                        ...
                        <div class="space-y-4">
                            <form
                                method="POST"
                                action="{{ route('admin.organizations.snapshots.store', $organization) }}"
                                class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"
                                x-data="{ periodType: '{{ old('period_type', 'monthly') }}' }"
                            >
                                @csrf
                                <div class="space-y-4">
                                    <div>
                                        <label class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('common.admin_organizations.snapshots.field.period_type') }}</label>
                                        <select
                                            name="period_type"
                                            x-model="periodType"
                                            class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
                                        >
                                            <option value="monthly">{{ __('common.admin_organizations.snapshots.period_type.monthly') }}</option>
                                            <option value="quarterly">{{ __('common.admin_organizations.snapshots.period_type.quarterly') }}</option>
                                            <option value="yearly">{{ __('common.admin_organizations.snapshots.period_type.yearly') }}</option>
                                        </select>
                                        @error('period_type')<p class="text-xs text-rose-500">{{ $message }}</p>@enderror
                                    </div>
                                    <div>
                                        <label class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('common.admin_organizations.snapshots.field.year') }}</label>
                                        <input
                                            type="number"
                                            name="year"
                                    ...
                                    <div class="grid gap-3 sm:grid-cols-2" x-show="periodType === 'monthly'" x-cloak>
                                        <div>
                                            <label class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('common.admin_organizations.snapshots.field.month') }}</label>
                                            <select
                                                name="month"
                                                class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
                                            >
                                                <option value=""></option>
                                                @foreach(range(1, 12) as $month)
                                                    <option value="{{ $month }}" @selected(old('month') == $month)>{{ \DateTime::createFromFormat('!m', $month)->format('F') }}</option>
                                                @endforeach
                                            </select>
                                            @error('month')<p class="text-xs text-rose-500">{{ $message }}</p>@enderror
                                        </div>
                                    </div>
                                    <div class="grid gap-3 sm:grid-cols-2" x-show="periodType === 'quarterly'" x-cloak>
                                        <div>
                                            <label class="text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('common.admin_organizations.snapshots.field.quarter') }}</label>
                                            <select
                                                name="quarter"
                                                class="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"
                                            >
                                                ...
                                                @foreach(range(1, 4) as $quarter)
                                                    <option value="{{ $quarter }}" @selected(old('quarter') == $quarter)>{{ $quarter }}</option>
                                                @endforeach
                                            </select>
                                            @error('quarter')<p class="text-xs text-rose-500">{{ $message }}</p>@enderror
                                        </div>
                                    </div>
                                    <div class="flex justify-end">
                                        <button type="submit" class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-slate-800 transition">
                                            {{ __('common.admin_organizations.snapshots.submit') }}
                                        </button>
                                    </div>
                                </div>
                            </form>
                            ...
                            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                <div class="flex items-center justify-between">
                                    <h2 class="text-sm font-semibold text-slate-900">{{ __('common.admin_organizations.snapshots.heading') }}</h2>
                                    <span class="text-xs uppercase tracking-wide text-slate-400">{{ __('common.admin_organizations.snapshots.latest', ['count' => $snapshots->count()]) }}</span>
                                </div>
                                <div class="mt-4 space-y-3">
                                    @forelse($snapshots as $snapshot)
                                        <div class="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                                            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                <div>
                                                    <p class="text-sm font-semibold text-slate-900">{{ $snapshot->period_label }}</p>
                                                    <p class="text-xs text-slate-500">{{ $snapshot->created_at->diffForHumans() }}</p>
                                                </div>
                                                <div class="flex flex-wrap gap-3 text-xs font-semibold text-slate-600">
                                                    <span>{{ __('common.admin_organizations.metrics.male') }} {{ number_format($snapshot->totals['male'] ?? 0) }}</span>
                                                    <span>{{ __('common.admin_organizations.metrics.female') }} {{ number_format($snapshot->totals['female'] ?? 0) }}</span>
                                                    <span>{{ __('common.admin_organizations.metrics.other') }} {{ number_format($snapshot->totals['other'] ?? 0) }}</span>
                                                    <span class="text-slate-500">{{ __('common.admin_organizations.metrics.total') }} {{ number_format($snapshot->totals['total'] ?? 0) }}</span>
                                                </div>
                                            </div>
                                            <div class="mt-3 flex items-center justify-between">
                                                <a href="{{ route('admin.organizations.snapshots.show', [$organization, $snapshot]) }}" class="text-xs font-semibold uppercase tracking-wide text-blue-600 hover:text-blue-500">
                                                    {{ __('common.admin_organizations.snapshots.download') }}
                                                </a>
                                            </div>
                                        </div>
                                    @empty
                                        <p class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">{{ __('common.admin_organizations.snapshots.empty') }}</p>
                                    @endforelse
                                </div>
                            </div>
