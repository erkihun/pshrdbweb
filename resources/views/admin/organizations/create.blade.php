...
@section('content')
    <div class="space-y-6">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-slate-900">{{ __('common.admin_organizations.actions.create') }}</h1>
            <p class="text-sm text-slate-500">{{ __('common.admin_organizations.list.create_description') }}</p>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.organizations.store') }}" class="space-y-6">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 transition">{{ __('common.actions.save') }}</button>
                </div>
            </form>
        </div>
    </div>
