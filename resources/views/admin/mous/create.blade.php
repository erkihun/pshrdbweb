...
@section('content')
    <div class="flex flex-col gap-6">
        <div class="flex flex-wrap items-center justify-between gap-4 rounded-xl bg-gradient-to-r from-slate-900 to-slate-800 p-6 text-white shadow-lg">
                <div>
                    <h1 class="text-2xl font-bold">{{ __('mous.create.title') }}</h1>
                    <p class="text-sm text-slate-200">{{ __('mous.create.description') }}</p>
                </div>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route('admin.mous.store') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @include('admin.mous._form')
                <div class="flex justify-end">
                    <button type="submit" class="btn-primary">
                        {{ __('mous.create.submit') }}
                    </button>
                </div>
            </form>
        </div>
