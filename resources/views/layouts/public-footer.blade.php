...
$privacyUrl = $footerMeta['privacy_policy_url'] ?? $footerMeta['privacy_policy'] ?? $footer['privacy_policy_url'] ??
$siteSettings['privacy_policy_url'] ?? route('privacy');
$termsUrl = $footerMeta['terms_service_url'] ?? $footerMeta['terms_url'] ?? $footerMeta['terms_of_service_url'] ??
$footer['terms_service_url'] ?? $siteSettings['terms_service_url'] ?? '#';
$accessibilityUrl = $footerMeta['accessibility_url'] ?? $footerMeta['accessibility'] ?? $footer['accessibility_url'] ??
$siteSettings['accessibility_url'] ?? '#';

// Social media links (you might want to add these to your settings)
$socialLinks = [
'facebook' => $footerMeta['facebook_url'] ?? $footerMeta['facebook'] ??
'https://web.facebook.com/profile.php?id=100067771711638',
...
<footer class="relative overflow-hidden bg-gradient-to-b from-brand-ink via-brand-ink to-gray-900 text-white">
    ...
    <div class="absolute inset-0 overflow-hidden">
        ...
        <div class="absolute -bottom-40 -left-40 w-80 h-80 bg-purple-500/5 rounded-full blur-3xl animate-pulse-slow"
    ...
    <div class="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div class="grid gap-10 lg:grid-cols-4 lg:gap-8">
            ...
            <div class="lg:col-span-2">
                <div class="space-y-6">
                    ...
                    <div class="flex items-center space-x-3">
                        <div
                            class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-brand-gold to-amber-500 shadow-lg transform hover:scale-105 transition-transform duration-300">
                    ...
                    <div class="space-y-4">
                        <a href="tel:{{ $phone }}"
                            class="group flex items-center space-x-3 text-gray-300 hover:text-orange-600 transition-colors duration-300">
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-lg bg-white/5 backdrop-blur-sm group-hover:bg-white/10 transition-all duration-300">
                    ...
                    {{-- Social Media Links --}}
                    <div class="pt-4">
                        <h4 class="mb-4 text-sm font-semibold uppercase tracking-wider text-white/90">
                            {{ __('public.footer.follow_us') }}</h4>
                        @php
                        $socialIcons = [
                        'facebook' => '<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill-rule="evenodd"
                                d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z"
                                clip-rule="evenodd" />
                        </svg>',
                        'tiktok' => '<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path
                                d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
                        </svg>',
                        'telegram' => '<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path
                        ...
                        'linkedin' => '<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path
                                d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                        </svg>',
                        'twitter' => '<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path
                                d="M8.29 20.251c7.547 0 11.675-6.253 11.675-11.675 0-.178 0-.355-.012-.53A8.348 8.348 0 0022 5.92a8.19 8.19 0 01-2.357.646 4.118 4.118 0 001.804-2.27 8.224 8.224 0 01-2.605.996 4.107 4.107 0 00-6.993 3.743 11.65 11.65 0 01-8.457-4.287 4.106 4.106 0 001.27 5.477A4.072 4.072 0 012.8 9.713v.052a4.105 4.105 0 003.292 4.022 4.095 4.095 0 01-1.853.07 4.108 4.108 0 003.834 2.85A8.233 8.233 0 012 18.407a11.616 11.616 0 006.29 1.84" />
                        </svg>',
                        'instagram' => '<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill-rule="evenodd"
                        ...
                        'youtube' => '<svg class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path fill-rule="evenodd"
            ...
            <div>
                <h3
                    class="mb-6 text-sm font-semibold uppercase tracking-wider text-white/90 after:ml-2 after:inline-block after:h-0.5 after:w-8 after:bg-gradient-to-r after:from-brand-gold after:to-amber-500">
                ...
                <ul class="space-y-3">
                    ...
                    <li>
                        <a href="{{ $resolvedUrl }}" @if($isExternal) target="_blank" rel="noopener noreferrer" @endif
                            ...
                            {{-- Icon --}}
                            <span
                                class="flex h-7 w-7 items-center justify-center rounded-md bg-white/5 group-hover:bg-gradient-to-br group-hover:from-brand-gold group-hover:to-amber-500 transition-all duration-300">
                                <x-heroicon-o-arrow-top-right-on-square
                                    class="h-4 w-4 text-gray-400 group-hover:text-white transition-colors duration-300"
                                    aria-hidden="true" />
                            </span>
            ...
            <div>
                ...
                <div class="rounded-xl bg-white/5 backdrop-blur-sm p-4 border border-white/10">
                    <div class="flex items-start space-x-3">
                        <div
                            class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-brand-gold/20 to-amber-500/20">
                            <x-heroicon-o-clock class="h-5 w-5 text-orange-600" aria-hidden="true" />
                        </div>
                        <div>
                            <p class="text-sm leading-relaxed text-gray-300">
                                {{ __('public.footer.office_hours_value') }}</p>
                ...
                <div class="mt-8">
                    <h4 class="mb-4 text-sm font-semibold uppercase tracking-wider text-white/90">
                        {{ __('public.footer.newsletter') }}</h4>
                    <form class="space-y-3" id="newsletterForm">
                        <div class="relative">
                            <input type="email" placeholder="{{ __('public.footer.email_placeholder') }}"
        ...
        <button onclick="window.scrollTo({top: 0, behavior: 'smooth'})"
            ...
            aria-label="{{ __('public.buttons.back_to_top') }}" id="backToTop"
