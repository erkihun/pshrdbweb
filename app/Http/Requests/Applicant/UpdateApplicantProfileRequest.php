...
class UpdateApplicantProfileRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'education_document' => ['nullable', 'file', 'mimes:pdf', 'max:2048'],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            ...
            if ($applicantId) {
                $application = VacancyApplication::where('applicant_id', $applicantId)
                    ->with('vacancy')
                    ->latest()
                    ->first();

                if ($application && $application->vacancy?->deadline && now()->gt($application->vacancy->deadline)) {
                    throw new AuthorizationException('Profile updates are locked after the vacancy deadline.');
                }
            }
        });
    }
